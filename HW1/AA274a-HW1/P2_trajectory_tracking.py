...
class TrajectoryTracker:
    ...
    def compute_control(self, x: float, y: float, th: float, t: float) -> T.Tuple[float, float]:
        ...
        B = np.array([xdd_d + self.kpx * (x_d - x) + self.kdx * (xd_d - xdot), ydd_d + self.kpy * (y_d - y) + self.kdy * (yd_d - ydot)])

        results = np.linalg.solve(J, B)

        om = results[1]
        V = results[0] * dt + self.V_prev
        # V = np.trapz(results[0], dx=dt)
