...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        insidemap = (self.statespace_lo[0] <= x[0] <= self.statespace_hi[0] * self.statespace_lo[1] <= x[1] <= self.statespace_hi[1])
        inobject = self.occupancy.is_free(x)
        # Return Boolean depending on True or False:
        return insidemap * inobject
    ...
    def get_neighbors(self, x):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if i == 0 and j == 0:
                    new = (x[0] + i * self.resolution, x[1] + j * self.resolution)
                    neighbor = self.snap_to_grid(new)
    ...
    def solve(self):
        ...
        HINTS:  We're representing the open and closed sets using python's built-in
                ...
                set membership efficientclly using the syntax "if item in set".
        """
        ########## Code starts here ##########
        ...
        while len(self.open_set) > 0:
            ...
            for neighbor in self.get_neighbors(x):
                if neighbor in self.closed_set:
                    continue
                tentative_cost_to_arrive = self.cost_to_arrive[x] + self.distance(x, neighbor)
