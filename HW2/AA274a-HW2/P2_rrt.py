    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for i in range(max_iters):
            ...
            if z < goal_bias:
                ...
                xrand = np.random.uniform(self.statespace_lo, self.statespace_hi)
            
            xnear = V[self.find_nearest(V[:n], xrand)]
            xnew = self.steer_towards(xnear, xrand, eps)

            if self.is_free_motion(self.obstacles, xnear, xnew):
                V[n] = xnew
                P[n] = self.find_nearest(V[:n], xnear)
                n += 1

                if (np.linalg.norm(xnew - self.x_goal) < eps):
                    success = True
                    
                    self.path = [self.x_goal]
                    # We can fill the path in reverse order since we now know the parents of the nodes
                    while (self.path[-1] != self.x_init).all():
                        self.path.append(V[P[self.find_nearest(V[:n], self.path[-1])]])
                    self.path.reverse()
